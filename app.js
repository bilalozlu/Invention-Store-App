...
app.delete('/users/:id', function (request, response) {
    inventions.updateMany({user_id: request.params.id}, {$set: {hide: true}});
...
app.post('/inventions/exhibit/:user_id/:user_name', (request, response) => {
    inventions.insertOne(Object.assign(request.body, { 'numberOfRates': 0, 'sumOfRates': 0,'rates': [],
        'user_id': request.params.user_id, 'user_name': request.params.user_name, hide: false}),
     (error, result) => {
        try {
...
// app.get('/inventions', async function (request, response) {
//     inventions.find({}).toArray((error, result) => {
//         try {
...
app.put('/inventions/rate/:rater_id/:user_id/:id/:oldRate', (request, response) => {
    ...
    if (addNew == 0) {
        inventions.updateOne({_id: new mongodb.ObjectId(request.params.id), "rates.rater_id": request.params.rater_id},
            {$inc: {"rates.$.rate": addRate}})
    }
    else {
        inventions.updateOne(
            {_id: new mongodb.ObjectId(request.params.id)},
            {$push:  {rates: {rater_id: request.params.rater_id, rate: addRate}}})
    }

    users.updateOne({_id: new mongodb.ObjectId(request.params.user_id)},
        {$inc: { sumOfRates: addRate, numberOfRates: addNew}})
